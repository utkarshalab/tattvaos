%ifndef GUARD_LIB_UMATH_SCALAR_MIN_F32_ASM
%define GUARD_LIB_UMATH_SCALAR_MIN_F32_ASM
; =============================================================================
; umath - unified math library
; scalar/min_f32.asm - single-precision float minimum implementations
; =============================================================================
...
;   - Strict IEEE-754 minimum (umath_min_f32_ieee):
...
; -----------------------------------------------------------------------------
; umath_min_f32 - scalar single-precision float minimum
; args:    xmm0 = a
;          xmm1 = b
; returns: xmm0 = min(a, b)
; -----------------------------------------------------------------------------
global umath_min_f32
umath_min_f32:
    minss   xmm0, xmm1
    ret

; -----------------------------------------------------------------------------
; umath_min_f32_ieee - strict IEEE-754 compliant float minimum
; args:    xmm0 = a
;          xmm1 = b
; returns: xmm0 = min(a, b) matching NaN propagation and signed zero rules
; -----------------------------------------------------------------------------
global umath_min_f32_ieee
umath_min_f32_ieee:
    ucomiss xmm0, xmm1
...
; -----------------------------------------------------------------------------
; umath_min_f32_array - compute element-wise minimum of two float arrays
...
umath_min_f32_array:
    ...
    mov     r8, rcx
    shr     r8, 4               ; count of 16-float (64-byte) blocks
...
; -----------------------------------------------------------------------------
; umath_min_f32_array_inplace - compute in-place element-wise minimum of two arrays
; args:    rdi = destination/source array pointer (dst_src)
;          rsi = source pointer b (src_b)
;          rdx = size of arrays (count)
; returns: void
; -----------------------------------------------------------------------------
global umath_min_f32_array_inplace
umath_min_f32_array_inplace:
    test    rdi, rdi
    jz      .done
    test    rsi, rsi
    jz      .done
...
; -----------------------------------------------------------------------------
; umath_min_f32_reduce - reduce an array of floats to its minimum value
; args:    rdi = source pointer (src)
;          rsi = size of array (count)
; returns: xmm0 = minimum float value in array (returns NaN if any element is NaN)
; -----------------------------------------------------------------------------
global umath_min_f32_reduce
umath_min_f32_reduce:
    xorps   xmm0, xmm0
    test    rdi, rdi
    jz      .done
    test    rsi, rsi
...
; -----------------------------------------------------------------------------
; umath_min_f32_reduce_index - reduce array of floats to index of minimum (argmin)
; args:    rdi = source pointer (src)
;          rsi = size of array (count)
; returns: rax = index of first occurrence of the minimum value, or -1 if empty
; -----------------------------------------------------------------------------
global umath_min_f32_reduce_index
umath_min_f32_reduce_index:
    mov     rax, -1
    test    rdi, rdi
    jz      .done
    test    rsi, rsi
...
; -----------------------------------------------------------------------------
; umath_min_f32_inplace_clip - clips elements in-place to not exceed threshold (top limit)
; args:    rdi = buffer pointer (buf)
;          rsi = size of array (count)
;          xmm0 = threshold value
; returns: void
; -----------------------------------------------------------------------------
global umath_min_f32_inplace_clip
umath_min_f32_inplace_clip:
    test    rdi, rdi
    jz      .done_clip
    test    rsi, rsi
    jz      .done_clip
...
; -----------------------------------------------------------------------------
; umath_min_f32_reduce_range - reduce sub-segment of array to its minimum value
; args:    rdi = source pointer (src)
;          rsi = start index
;          rdx = end index (exclusive)
; returns: xmm0 = minimum value in range, or 0 if invalid
; -----------------------------------------------------------------------------
global umath_min_f32_reduce_range
umath_min_f32_reduce_range:
    xorps   xmm0, xmm0
    test    rdi, rdi
    jz      .done_range
    cmp     rsi, rdx
...
%endif ; GUARD_LIB_UMATH_SCALAR_MIN_F32_ASM
