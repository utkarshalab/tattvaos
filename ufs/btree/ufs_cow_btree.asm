...
;
; Implements:
;   - Binary search across sorted node key entries
...
extern ufs_ag_alloc_block

; -----------------------------------------------------------------------------
; ufs_btree_init_node
;
; Initializes a fresh CoW B-tree node header.
; -----------------------------------------------------------------------------
align 32
ufs_btree_init_node:
    push rbx

    mov rbx, rdi                    ; Pointer to ufs_btree_node_t
    mov dword [rbx + ufs_btree_node_t.flags], esi  ; Leaf or Internal
...
; ufs_btree_lookup
;
; Performs a binary search across a sorted B-tree node to locate a key.
;
; Inputs:
...
.binary_search_loop:
    ...
    mov r10d, r9d
    add r10d, r8d
    shr r10d, 1                     ; R10D = mid index = (low + high) / 2
...
; ufs_btree_insert
;
; Inserts a new key-value entry into a B-tree node using Copy-on-Write (CoW).
;
; Inputs:
;   RDI = Pointer to current ufs_btree_node_t
;   RSI = 64-bit Key to insert
;   RDX = 64-bit Physical Block Address pointer
;
; Returns:
;   RAX = Pointer to new CoW-duplicated B-tree node (or 0 if memory full)
; -----------------------------------------------------------------------------
align 32
ufs_btree_insert:
    push rbx
    ...
    mov rbx, rdi                    ; RBX = old node
    mov r12, rsi                    ; R12 = key
    mov r13, rdx                    ; R13 = block_ptr

    mov ecx, [rbx + ufs_btree_node_t.key_count]
    cmp ecx, UFS_BTREE_MAX_KEYS
    jge .node_full_split

    ; Append key entry to leaf
    ...
    mov rax, rbx                    ; Return updated node pointer
...
.node_full_split:
    ; Node split handler (creates sibling node)
    mov rax, rbx
    pop r14
    pop r13
    pop r12
...
; ufs_btree_self_heal
;
; Verifies BLAKE3 checksum of node. On corruption, reads mirror AG replica.
;
; Inputs:
;   RDI = Pointer to ufs_btree_node_t
;   RSI = Mirror Allocation Group replica block ID
;
; Returns:
;   EAX = 0 (Repaired successfully) or -1 (Unrecoverable corruption)
; -----------------------------------------------------------------------------
align 32
ufs_btree_self_heal:
    push rbx
