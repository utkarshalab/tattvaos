...
; =============================================================================
; HTTP/1.1 Web Server Engine with Direct UFS Zero-Copy Static File Streaming.
;
; Delegates:
;   - Static Asset File IO Read      -> storage/ufs/vfs/ & storage/ufs/ufs.asm
...
global http1_stream_ufs_file

extern ufs_read_file
extern ufs_get_file_size
...
; -----------------------------------------------------------------------------
; http1_stream_ufs_file — Direct Zero-Copy Streaming from UFS to HTTP Socket
; Input: RDI = Pointer to URI Path String (e.g., "/var/www/index.html")
;        RSI = Socket File Descriptor
; -----------------------------------------------------------------------------
align 32
http1_stream_ufs_file:
    push rbp
    mov rbp, rsp
    push rbx

    mov rbx, rdi
    ; Query file size directly from Universal File System (UFS)
    call ufs_get_file_size

    ; Stream UFS storage blocks directly into socket TX DMA buffer
    call ufs_read_file
