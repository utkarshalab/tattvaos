; =============================================================================
; Tattva OS — ufs/extents/extents.asm
; =============================================================================
...
;   - Extent node insertion & contiguous extent merging (`ufs_extent_insert`)
...
;
; Inputs:
;   RDI = Pointer to 64-byte extent tree node header (`ufs_extent_header_t`)
...
ufs_extent_map_block:
    ...
    cmp word [rbx + ufs_extent_header_t.eh_magic], UFS_EXTENT_MAGIC
    jne .corrupt_extent

    movzx ecx, word [rbx + ufs_extent_header_t.eh_entries]
    test ecx, ecx
    jz .unallocated_hole            ; No entries

    lea rbx, [rbx + ufs_extent_header_t_size]  ; RBX = first extent entry

.extent_search_loop:
    test ecx, ecx
    jz .unallocated_hole

    mov eax, [rbx + ufs_extent_t.ee_block]     ; Start block of extent
    movzx edx, word [rbx + ufs_extent_t.ee_len]; Extent length in blocks
    ...
    movzx rax, word [rbx + ufs_extent_t.ee_start_hi]
    shl rax, 32
    or eax, dword [rbx + ufs_extent_t.ee_start_lo]

    ; Physical LBA = extent_start_phys + (target_block - extent_start_logical)
    sub r12d, [rbx + ufs_extent_t.ee_block]
    add rax, r12
...
ufs_extent_insert:
    ...
    mov rbx, rdi
    movzx eax, word [rbx + ufs_extent_header_t.eh_entries]
    inc word [rbx + ufs_extent_header_t.eh_entries]
