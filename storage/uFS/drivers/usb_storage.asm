...
global ufs_usb_build_scsi_read10
global ufs_usb_build_scsi_write10
global ufs_usb_storage_read
global ufs_usb_storage_write

; -----------------------------------------------------------------------------
; ufs_usb_build_scsi_read10
;
...
ufs_usb_build_scsi_read10:
    ...
    mov rbx, rdi
    mov dword [rbx + ufs_usb_cbw_t.dCBWSignature], USB_CBW_SIGNATURE
    mov [rbx + ufs_usb_cbw_t.dCBWTag], ecx

    mov eax, edx
    shl eax, 9                      ; Bytes to transfer = count * 512
    mov [rbx + ufs_usb_cbw_t.dCBWDataTransferLength], eax
    mov byte [rbx + ufs_usb_cbw_t.bmCBWFlags], 0x80   ; Direction: IN (Device->Host)
    mov byte [rbx + ufs_usb_cbw_t.bCBWLUN], 0
    mov byte [rbx + ufs_usb_cbw_t.bCBWCBLength], 10    ; 10-byte SCSI CDB

    ; Construct SCSI READ(10) CDB
    lea rbx, [rbx + ufs_usb_cbw_t.CBWCB]
...
ufs_usb_build_scsi_write10:
    ...
    shl eax, 9
    mov [rbx + ufs_usb_cbw_t.dCBWDataTransferLength], eax
    mov byte [rbx + ufs_usb_cbw_t.bmCBWFlags], 0x00   ; Direction: OUT (Host->Device)
    mov byte [rbx + ufs_usb_cbw_t.bCBWLUN], 0
    mov byte [rbx + ufs_usb_cbw_t.bCBWCBLength], 10
...
align 32
ufs_usb_storage_read:
    call ufs_usb_build_scsi_read10
    ret
...
align 32
ufs_usb_storage_write:
    call ufs_usb_build_scsi_write10
    ret
